...
8. [Interpretazione delle Metriche](#interpretazione-delle-metriche)
...
Questa sezione spiega in dettaglio come interpretare le metriche del modello ML nel contesto della produzione industriale.

### R² (Coefficiente di Determinazione)

**Cos'è:** Misura quanta varianza dei dati reali viene "spiegata" dal modello. Va da 0 a 1 (può essere negativo in casi estremi).

**Formula concettuale:**
```
R² = 1 - (Errori del modello / Varianza totale dei dati)
```

**Tabella di Interpretazione:**

| R² | Significato | Azione Consigliata |
|----|-------------|-------------------|
| **> 0.9** | Eccellente - il modello cattura quasi tutti i pattern | Usa ML con fiducia |
| **0.7 - 0.9** | Buono - predizioni affidabili | Usa ML |
| **0.5 - 0.7** | Discreto - utile ma con cautela | Usa approccio Hybrid |
| **0.3 - 0.5** | Scarso - molti pattern non catturati | Preferisci Database |
| **< 0.3** | Insufficiente - modello quasi inutile | Rivedi features/dati |

**Esempio pratico:**
```
Se R² = 0.85 significa che il modello spiega l'85% della variabilità 
nella produzione. Il restante 15% è dovuto a fattori non catturati 
(es. guasti imprevisti, variazioni materiali, fattore umano).
```
...
**Cos'è:** L'errore medio nelle stesse unità di misura del target. Penalizza maggiormente gli errori grandi.

**Formula:**
```
RMSE = √(media degli errori al quadrato)
     = √(Σ(predetto - reale)² / n)
```

**Tabella di Interpretazione per Target:**

| Target | RMSE Buono | RMSE Accettabile | RMSE Critico |
|--------|------------|------------------|--------------|
| **Quantity** (es. 100-500 pezzi) | < 20 | 20-50 | > 50 |
| **Energy** (es. 50-200 kWh) | < 10 | 10-30 | > 30 |
| **Scrap** (es. 0-20 pezzi) | < 3 | 3-7 | > 7 |

**Esempio pratico:**
```
Se predici quantity con RMSE = 25:
- Produzione reale: 150 pezzi
- Predizione tipica: tra 125 e 175 pezzi (±25)

Un RMSE di 25 su una produzione media di 150 = errore ~17%
Un RMSE di 25 su una produzione media di 500 = errore ~5%
```

**Tip:** Confronta sempre RMSE con la media del target per avere un'idea relativa.

---

### MAPE (Mean Absolute Percentage Error)

**Cos'è:** L'errore medio in percentuale. Indipendente dalla scala, quindi facilmente confrontabile.

**Formula:**
```
MAPE = media(|predetto - reale| / reale × 100)
```

**Tabella di Interpretazione:**

| MAPE | Significato | Nel contesto produzione |
|------|-------------|------------------------|
| **< 5%** | Eccellente | Pianificazione precisa possibile |
| **5-10%** | Buono | Affidabile per decisioni operative |
| **10-15%** | Accettabile | Utile come indicazione generale |
| **15-25%** | Scarso | Solo per stime approssimative |
| **> 25%** | Insufficiente | Non affidabile |

**Esempio pratico:**
```
MAPE = 8% significa:
- Se prevedi 100 pezzi, il valore reale sarà tipicamente tra 92 e 108
- Su una settimana di predizioni, l'errore medio è dell'8%
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   R² alto + RMSE basso + MAPE basso = MODELLO OTTIMO ✓         │
│                                                                 │
│   R² alto + RMSE alto = Possibile overfitting o outliers       │
│                         (il modello "memorizza" i dati)         │
│                                                                 │
│   R² basso + RMSE basso = Dati con poca varianza               │
│                           (produzione molto costante)           │
│                                                                 │
│   R² basso + RMSE alto = Modello inadeguato                    │
│                          (serve più training o features)        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
### Come Leggere i Risultati nel Sistema

Quando chiami `GetSmartPredictionAsync` o `GetComparisonPredictionAsync`, ottieni:

```csharp
var result = await service.GetSmartPredictionAsync(tenantCode, args);

// Metriche disponibili:
Console.WriteLine($"R²: {result.QualityMetrics?.R2Score}");
Console.WriteLine($"RMSE: {result.QualityMetrics?.RMSE}");
Console.WriteLine($"Confidenza: {result.Confidence}");  // Basata su R²
```
...
=== PREDIZIONE SMART ===
Target: quantity
Fonte Selezionata: Hybrid

Metriche Modello:
  R² = 0.72  → Discreto, cattura il 72% dei pattern
  RMSE = 35  → Errore medio di ±35 pezzi
  
Predizione:
  Database: 420 pezzi (media mobile)
  ML Model: 385 pezzi
  Hybrid: 395 pezzi (70% ML + 30% DB)
  
Confidenza: 58% → Riflette l'incertezza della zona ibrida

INTERPRETAZIONE:
- Il modello è discreto ma non eccellente
- C'è una differenza dell'8% tra DB e ML
- La predizione ibrida bilancia entrambe le fonti
- Consiglio: usare 395 ma pianificare per un range 360-430
```

---

### Dashboard Mentale per Decisioni Rapide
...
├────────────────────────────────────────────────────────────────┤
│                                                                 │
│  "Posso fidarmi della predizione ML?"                          │
│                                                                 │
│  ┌─── R² > 0.75? ───┐                                          │
│  │                  │                                          │
│  │  SÌ             NO                                          │
...
│                                                                 │
│  VERIFICA AGGIUNTIVA:                                          │
│  - MAPE < 10%? → OK per decisioni operative                    │
│  - MAPE > 15%? → Solo per stime, non per pianificazione        │
│                                                                 │
└────────────────────────────────────────────────────────────────┘
```
...
### Interpretare un Backtest

Quando esegui `RunBacktestAsync`, ottieni un report come questo:
...
**Tenant:** TENANT-001
**Periodo:** 30 giorni
**Predizioni totali:** 25

## Metriche di Errore (MAPE)

| Metodo   | MAPE   | Mediana | Min   | Max    | StdDev |
|----------|--------|---------|-------|--------|--------|
| Database | 12.5%  | 11.2%   | 2.1%  | 28.4%  | 6.8    |
| ML Model | 9.8%   | 8.5%    | 1.5%  | 22.1%  | 5.2    |
| Smart    | 8.2%   | 7.1%    | 1.2%  | 18.5%  | 4.1    |
```
...
|--------------|-------------|
| Smart MAPE (8.2%) < DB MAPE (12.5%) | L'approccio smart migliora del 34% rispetto al solo DB |
| Smart MAPE < ML MAPE | L'ibrido funziona meglio del solo ML (probabilmente R² nella zona 0.5-0.75) |
| Max Error Smart (18.5%) < Max DB (28.4%) | Smart riduce i picchi di errore |
| StdDev Smart (4.1) più bassa | Predizioni più consistenti, meno variabilità |
...
|---------|----------------|--------|
| R² scende nel tempo | **Concept drift** - i pattern stanno cambiando | Ri-trainare il modello |
| RMSE improvvisamente alto | Outliers o cambio nei dati | Verificare qualità dati |
| MAPE > 20% costante | Modello inadeguato | Rivedere features |
| ML sempre peggio di DB | Modello non aggiunge valore | Aumentare dati/features |
| Differenza ML-DB > 25% | Forte disaccordo tra metodi | Investigare manualmente |
...
| **StdDev** | Standard Deviation - misura della dispersione dei valori |
| **Mediana** | Valore centrale quando i dati sono ordinati |
| **Outlier** | Valore anomalo che si discosta significativamente dalla norma |
